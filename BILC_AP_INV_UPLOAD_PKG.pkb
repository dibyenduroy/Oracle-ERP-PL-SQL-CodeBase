...
----------------------------------------------------------------------------------------------
      ...
      BEGIN
         SELECT COUNT(1)
           INTO l_po_record_count
           FROM po_headers_all pha
          WHERE 1 = 1
            AND pha.segment1 = p_po_number;

         IF l_po_record_count = 0 THEN
            lv_err_msg := lv_err_msg || ' ' || 'Invalid PO Number- ';
            lf_err_flag := 1;
         END IF;
         ...
         BEGIN
            SELECT s.concatenated_segment_delimiter
             ...
             WHERE s.application_id = a.application_id
               AND s.id_flex_code = 'GL#'--key_flex_code
               AND s.id_flex_num = (SELECT id_flex_num FROM fnd_id_flex_structures WHERE id_flex_code = 'GL#' AND UPPER(id_flex_structure_code)=UPPER('Infratel_COA'))--structure_number
               AND a.application_short_name = 'SQLGL';--get_delimiter.application_short_name;
